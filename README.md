...
| `--network` | Show configured path and Kubernetes-observed state (Service with nodePort/externalIPs/ipFamilies, EndpointSlice, Ingress/Route) for Pod, Deployment, ReplicaSet, StatefulSet, DaemonSet. External reachability is not determined by Service type alone |
