...
**Observed state (Phase 5):** MetalLB status resources (ServiceL2Status, ServiceBGPStatus) are fetched when their CRDs exist. `observedState` distinguishes: "advertising" (matching status exists), "configured" (status API available but no matching status), "unknown" (status API absent or unavailable). BGPPeer and BFDProfile are cross-referenced by (namespace, name) from ServiceBGPStatus.peers. BGP session state is always "unknown" when BGP advertisements or status are present — status represents advertisement intent, not session establishment. ConfigurationState resources show result/errorSummary/conditions when present. Service events are fetched per-service with UID validation and exact MetalLB component matching (metallb-speaker, metallb-controller). Each observation API's availability (available/absent/unavailable) is shown in JSON `apiAvailability`.

MetalLB LIST failures (403, 500, timeout) propagate as typed warnings with retry. MetalLB CRD absence is handled gracefully. JSON includes a `metallb` object per service path with `provider`, `requestedIPs`, `requestedPool`, `pools[]`, `advertisements[]`, `observation` (with `observedState`, `sessionState`, `apiAvailability`, `l2AdvertisedNodes`, `l2Interfaces`, `bgpNodeStatus`, `relatedPeers`, `relatedBfdProfiles`, `configurationState`, `events`), and `warnings[]`.

**MetalLB observed state (Phase 5):** When MetalLB status resources exist (`ServiceL2Status`, `ServiceBGPStatus`), the tool correlates them with the target service to determine the observed advertisement state. API availability is tracked explicitly as `Available` (CRD exists, LIST succeeded), `Absent` (CRD not found), or `Unavailable` (CRD exists but LIST failed). The `observedState` field is one of: `advertising` (status resources confirm the service is being advertised), `configured` (at least one status API is Available but no status resources match this service), `unknown` (a status API is Unavailable), or empty (both status APIs are Absent, no observation possible). `ServiceBGPStatus` uses `status.peers` (string array of BGPPeer names) for cross-referencing. `ServiceL2Status` uses `status.interfaces` (array of `{name: string}` InterfaceInfo objects). BGP peers (`BGPPeer`) and BFD profiles (`BFDProfile`) are cross-referenced by name within the MetalLB namespace. BGP `sessionState` is always `unknown` when BGP-related items are present, because status resources represent advertisement intent, not BGP session establishment. Service events from MetalLB components (metallb-controller, speaker) are fetched from the core v1 Event API and correlated by service name/namespace. All observation API fetches are conditional on CRD existence and failures are handled gracefully as typed warnings.
