...
MetalLB LIST failures (403, 500, timeout) propagate as typed warnings rather than being silently swallowed. MetalLB CRD absence is handled gracefully. JSON includes a `metallb` object per service path with `provider`, `requestedIPs`, `requestedPool`, `pools[]` (with `allocationMatch`, `namespace`, `labels`, `serviceAllocation`, `statusAvailableIPv4`, `statusAvailableIPv6`, `statusAssignedIPv4`, `statusAssignedIPv6`), `advertisements[]` (with `namespace`, `interfaces`, `peers`, `serviceSelectors`, `nodeSelectors`, `candidateNodes`, `nodeSelectorStatus`), `observation` (with `observedState`, `l2AdvertisedNodes`, `bgpNodeStatus`, `relatedPeers`, `relatedBfdProfiles`), and `warnings[]`.

**MetalLB observed state (Phase 5):** When MetalLB status resources exist (`ServiceL2Status`, `ServiceBGPStatus`), the tool correlates them with the target service to determine the observed advertisement state. The `observedState` field is one of: `advertising` (status resources confirm the service is being advertised), `configured` (advertisements exist but no status resources match this service), or `unknown` (status CRDs are not present on the cluster). BGP peers (`BGPPeer`) and BFD profiles (`BFDProfile`) are cross-referenced by peer address and profile name respectively. **Important:** `ServiceBGPStatus` represents advertisement intent, not actual BGP session establishment. BGP session state (up/down) cannot be determined from these resources alone. All observation API fetches are conditional on CRD existence and failures are handled gracefully as typed warnings.
